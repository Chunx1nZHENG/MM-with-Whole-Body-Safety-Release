...
class QuadraticCostF : public problem::CostFunction {
 public:
  QuadraticCostF(const MatrixXd& Q, const MatrixXd& R, const MatrixXd& H, const VectorXd& q,
  ...
  static QuadraticCostF LQRCost(const MatrixXd& Q, const MatrixXd& R, const VectorXd& xref,
    ...
    VectorXd q = -(Q * xref);
    VectorXd r = -(R * uref);
    double c = 0.5 * xref.dot(Q * xref);
    return QuadraticCostF(Q, R, H, q, r, c, terminal);
  }
...
class QuadraticCostF_test : public problem::CostFunction {
 public:
  QuadraticCostF_test(const MatrixXd& Q, const MatrixXd& R, const MatrixXd& H, const VectorXd& xref,
                const VectorXd& uref, double c = 0, bool terminal = false)
      : n_(xref.size()),
        m_(uref.size()),
        isblockdiag_(H.norm() < 1e-8),
        Q_(Q),
  ...
  static QuadraticCostF_test LQRCost(const MatrixXd& Q, const MatrixXd& R, const VectorXd& xref,
                               const VectorXd& uref, bool terminal = false) {
    ...
    MatrixXd H = MatrixXd::Zero(n, m);
    double c = 0.5 * xref.dot(Q * xref);
    return QuadraticCostF_test(Q, R, H, xref, uref, c, terminal);
  }
