...
#include "Altro_control/Altro_model_joint4.hpp"
...
void MobileManipulator_joint4::Evaluate(const VectorXdRef& x, const VectorXdRef& u, Eigen::Ref<VectorXd> out){
//   std::cout << "joint4" << std::endl;  
...
void MobileManipulator_joint4::Jacobian(const VectorXdRef& x, const VectorXdRef& u, Eigen::Ref<MatrixXd> jac) {
  using CppAD::sin;
  using CppAD::cos;
...
void MobileManipulator_joint4::Hessian(const VectorXdRef& x, const VectorXdRef& u, const VectorXdRef& b, Eigen::Ref<MatrixXd> hess) {
  ALTRO_UNUSED(x);
  ALTRO_UNUSED(u);
  ALTRO_UNUSED(b);
  hess.setZero();
