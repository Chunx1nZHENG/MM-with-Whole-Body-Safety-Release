...
class MobileManipulatorest : public altro::problems::MobileManipulatorProblem, public ::testing::Test {
 protected:
  void SetUp() override { ; }
};
...
TEST_F(MobileManipulatorest, IncreasePenalty) {
...
TEST_F(MobileManipulatorest, SolveOneStep) {
  altro::augmented_lagrangian::AugmentedLagrangianiLQR<NStates, NControls>  solver_al = MakeALSolver();
  solver_al.SetTrajectory(std::make_shared<altro::Trajectory<NStates, NControls>>(InitialTrajectory()));
  solver_al.GetiLQRSolver().Rollout();
  solver_al.SetPenalty(10.0);
...
TEST_F(MobileManipulatorest, SolveConstrained) {
  altro::augmented_lagrangian::AugmentedLagrangianiLQR<NStates, NControls> solver_al = MakeALSolver();
  solver_al.SetPenalty(10.0);
...
TEST_F(MobileManipulatorest, SolveParallel) {
  N = 10;
  altro::augmented_lagrangian::AugmentedLagrangianiLQR<NStates, NControls> solver_al = MakeALSolver();
  // solver_al.SetPenalty(10.0);
...
// //build a ROS node to run the tests
// int main(int argc, char **argv) {
//   testing::InitGoogleTest(&argc, argv);
//   ros::init(argc, argv, "altro_test");
//   ros::NodeHandle nh;
