...
#include "Altro_control/Altro_model_joint3.hpp"
...
void MobileManipulator_joint3::Evaluate(const VectorXdRef& x, const VectorXdRef& u, Eigen::Ref<VectorXd> out){
      // std::cout << "joint3" << std::endl;  
...
void MobileManipulator_joint3::Jacobian(const VectorXdRef& x, const VectorXdRef& u, Eigen::Ref<MatrixXd> jac) {
  using CppAD::sin;
  using CppAD::cos;
...
void MobileManipulator_joint3::Hessian(const VectorXdRef& x, const VectorXdRef& u, const VectorXdRef& b, Eigen::Ref<MatrixXd> hess) {
  ALTRO_UNUSED(x);
  ALTRO_UNUSED(u);
  ALTRO_UNUSED(b);
  hess.setZero();
